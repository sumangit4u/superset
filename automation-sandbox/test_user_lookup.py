...
_MODULE_PATH = Path(__file__).resolve().parent / "user_lookup.py"
...
def test_get_user_by_username_is_injection_safe(conn: sqlite3.Connection) -> None:
    # A classic injection payload must be treated as a literal username and
    # match nothing rather than bypassing the WHERE clause.
...
def test_search_users_is_injection_safe(conn: sqlite3.Connection) -> None:
    # The payload is treated as a literal LIKE term, matching no users, and the
