...
class LogPruneCommand(BaseCommand):
    ...
    def run(self) -> None:
        ...
        # Log.dttm is stored as a naive UTC datetime (no tzinfo), so compute the
        # cutoff as a naive UTC datetime to avoid a naive/aware mismatch that
        # raises on PostgreSQL ("operator does not exist: timestamp without time
        # zone"). datetime.now(timezone.utc) replaces the deprecated
        # datetime.utcnow(); .replace(tzinfo=None) drops the tzinfo.
